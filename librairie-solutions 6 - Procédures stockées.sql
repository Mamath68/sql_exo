...
DELIMITER |
CREATE PROCEDURE livres_epuises()
BEGIN
    SELECT l.titre, c.nom as categorie
    FROM livres l
             INNER JOIN categories c ON l.id_categorie = c.id_categorie
    WHERE l.stock = 0
    ORDER BY c.nom, l.titre;
END |
DELIMITER ;
...
DELIMITER |
CREATE PROCEDURE livres_par_categorie(IN cat_id INT)
BEGIN
    SELECT l.titre, l.prix, l.stock
    FROM livres l
    WHERE l.id_categorie = cat_id
    ORDER BY l.titre;
END |
DELIMITER ;
...
CREATE PROCEDURE stats_editeur(
    IN p_editeur_id INT,
    OUT p_nb_livres INT,
    OUT p_prix_moyen DECIMAL(7, 2),
    OUT p_valeur_stock DECIMAL(7, 2)
)
BEGIN
    SELECT COUNT(*), ROUND(AVG(prix), 2), SUM(prix * stock)
    INTO p_nb_livres, p_prix_moyen, p_valeur_stock
    FROM livres
